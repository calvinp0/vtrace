 ...
 */
async function writeM199Repo(repoRoot: string, bystanders = 12): Promise<void> {
  await mkdir(path.join(repoRoot, "pkg"), { recursive: true });
...
async function primeM199Index(repoRoot: string, db: Database) {
...
test("M199: a one-file refresh writes the changed file's rows, not the repository's", async () => {
  await withFixture(async (repoRoot) => {
    await writeM199Repo(repoRoot);
    const db = openIndexerDatabase();
    try {
      const snapshot = await primeM199Index(repoRoot, db);
