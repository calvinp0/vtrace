...
//
// Two diagnoses, in precedence order:
//
...
// At most one capsule-level directive is emitted (the most specific that matched)
// — it is a hint about the bug class, not a per-line annotation.
 ...
 * intent is not a debugging one. At most one directive is returned — the most
 * specific that matched (guarded supersedes bare mutation).
 */
export function detectEditRiskDirectives(input: EditRiskInput): EditRiskDirective[] {
  ...
  // Guarded shared-state mutation — the specific, high-value diagnosis. The top
  // pivot is the primary edit site, so it is examined first by pivot order.
  ...
  // Bare shared-state mutation — the fallback when no guard is detectable.
