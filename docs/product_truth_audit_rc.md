...
### Post-RC subsystem wiring closures

Date: 2026-06-04

The following previously open subsystem-wiring items are now closed in code with regression coverage:

| Item                                         | Closing commit                                         | Current status                                                                                                                                                                                                                                  |
| -------------------------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Capsule-manifest staleness production wiring | `bdfe8a3 Wire memory and capsule staleness subsystems` | Closed; capsule manifests are persisted and surfaced (`capsuleManifestId` / `context.capsuleManifestId`) and checkable via `check-capsule` / `check_capsule_staleness` against a later index run.                                               |
| Session compression production trigger       | `bdfe8a3 Wire memory and capsule staleness subsystems` | Closed; a bounded, deterministic compression sweep now runs automatically after each successful `index` (`runBoundedSessionCompressionSweep`), with `compress-sessions` as the explicit counterpart. No always-on scheduler/daemon is required. |
| `search_logic_flow` call-edge traversal      | `9856f2d Add call-edge traversal to search_logic_flow` | Closed; traversal now includes statically resolved `calls` edges and coverage reports `callFlowEvidenceAvailable` / `callFlowEvidenceUsed`. `calls` edges cover Python, TypeScript, and Cython and remain conservative.                         |
...
| search logic flow      | Exact start/end FQNs, bounded deterministic static paths over `contains`/`imports`/statically resolved `calls` edges; rejects `cross_repo=true`.                                                                                          | Ready        | `src/logicFlow/searchLogicFlow.ts`, `src/mcp/tools.ts`, `src/logicFlow/searchLogicFlow.test.ts`                                                                          | No endpoint inference; `calls` edges cover Python, TypeScript, and Cython and stay conservative; `callFlowEvidenceAvailable` reports honest skip; no runtime/dataflow semantics. | Keep as specialist exact-FQN tool.                                                                  |
...
| session compression    | Compresses inactive sessions after default 2h; stores summary and consolidates repeated passive groups. Runs as a bounded deterministic sweep automatically after each successful `index`, plus the explicit `compress-sessions` command. | Ready        | `src/observations/sessionLifecycle.ts`, `src/runtime/reindexRepo.ts`, `src/cli/commands/compressSessionsCommand.ts`, `src/observations/observations.test.ts`             | Trigger is the post-index sweep; no always-on scheduler/daemon.                                                                                                                  | Keep docs explicit: post-index sweep + explicit command exist; no always-on scheduler is RC scope.  |
...
| Session compression              | Compresses inactive sessions and preserves durable memory; runs automatically as a bounded post-index sweep plus the explicit `compress-sessions` command. | True            | `sessionLifecycle.ts`, `reindexRepo.ts`                                     | “Post-index sweep + explicit command; no always-on scheduler requirement.”                |
...
| `search_logic_flow`                                                                                                                                                     | Ready                        | Exact FQN endpoint path search over `contains`/`imports`/statically resolved `calls` edges; coverage reports call-flow availability. `cross_repo=true` fails honestly. | Aligned.                                                                                  | No action.                                                                 |
...
| Session compression after inactivity                           | True; post-index sweep + explicit command | `sessionLifecycle.ts`, `reindexRepo.ts`                             | Bounded sweep after each index; no always-on session daemon.  |
