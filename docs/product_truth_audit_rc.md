...
| `search_logic_flow` call-edge traversal      | `9856f2d Add call-edge traversal to search_logic_flow` | Closed; traversal now includes statically resolved `calls` edges and coverage reports `callFlowEvidenceAvailable` / `callFlowEvidenceUsed`. `calls` edges remain Python-only and conservative.                                                  |
