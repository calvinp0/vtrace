...
Inactive sessions are compressed after the default two-hour inactivity threshold through two deterministic, bounded triggers: the explicit `vtrace compress-sessions <repo>` command, and a bounded sweep (up to 20 sessions) that runs automatically after each successful reindex (`vtrace index`, `index_repo`, watcher auto-reindex). There is no background daemon. Compression summarizes tool-call counts, touched files/symbols, key terms, and durable counts, and triggers conservative consolidation for repeated passive tool calls. Durable/manual observations remain visible, one-off passive observations are not consolidated below threshold, and compressed plus consolidated summaries stay searchable and stale-aware through lexical and structural signals. The default 90-day retention policy currently reports cleanup candidates; it does not silently delete durable data.

`get_context_capsule` (`capsuleManifestId`) and `run_pipeline` (`context.capsuleManifestId`) persist a deterministic capsule manifest and return its id. Feed that id to `check_capsule_staleness` or `vtrace check-capsule` to get a `fresh`/`stale` verdict against a later index run; modifying a linked file/symbol and reindexing makes the capsule stale.

Optional passive file awareness is available through `vtrace watch [repo]`. It is mark-stale-only by default: the watcher detects indexed source file creates/modifies/deletes, debounces bursts, records pending stale state, and leaves reindexing explicit. `vtrace watch [repo] --auto-reindex` opts into debounced automatic reindexing, prevents overlapping watcher-triggered index runs, and leaves failure/stale metadata visible if indexing fails. `index_status` and `run_pipeline.diagnostics.freshness` report watcher and auto-reindex state. After a successful reindex, existing file/symbol diffs drive conservative stale marking for linked observations and compressed session summaries.
...
- Treat watcher staleness, auto-reindex state, and anti-pattern observations as structural evidence only. They are not semantic rename detection, runtime tracing, intent inference, or project-rule generation.
