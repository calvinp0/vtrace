#!/usr/bin/env node
// Copies the repo-root bin/vexb launcher into the extension folder so that a
...
const SOURCE = path.join(REPO_ROOT, "bin", "vexb");
const TARGET_DIR = path.join(EXTENSION_DIR, "bin");
const TARGET = path.join(TARGET_DIR, "vexb");
